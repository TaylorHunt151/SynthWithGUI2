...
public:
    ...
    void OnKeyDown(wxKeyEvent& event) {
        ...
        if ((currentKey >= 'A' && currentKey <= 'Z') || (currentKey >= 0 && currentKey <= 1)) { //Checks if key is a valid input
	...
	void OnKeyUp(wxKeyEvent& event) {
        ...
        if ((currentKey >= 'A' && currentKey <= 'Z') || (currentKey >= 0 && currentKey <= 1)) { //Checks if key is a valid input
            ...
            for (int i = 0; i < voiceCount; i++) { //Find current key in noteSet vector and snuff it out

                if (noteSet[i] == currentKey) {
                    noteSet[i] = ' ';
                    noteSetter(i);
                }
...
private:
    void noteSetter(int oscNum) { //This sets the frequency of the oscillator. If no note keys are pressed, the amplitude goes to zero
        ...
        //keyInputManager manager; // Create an instance of KeyInputManager
